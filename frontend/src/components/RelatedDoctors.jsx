...
const RelatedDoctors = ({speciality,docId}) => {
    const {doctos} = useContext(AppContext);
  ...
  return (
    <div>
         <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
            ...
            <p className='sm:w-1/3 text-center text-sm'>simply browse through our extensive list of trusted doctor</p>
            <div className='w-full grid 
            grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {
                    relDoc.slice(0,5).map((item, index) => (
                        ...
                        <div key={index} onClick={()=> {navigate(`/appointment/${item._id}`);scrollTo(0,0)}} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
                            ...
                            <div className='p-4'>
                                <div
                                    className='flex items-center gap-2 text-sm text-center text-green-500 '>
